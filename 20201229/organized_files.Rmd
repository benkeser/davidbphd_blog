...
We see that the `report` depends on the source file `code/report.Rmd` as well as some files in the `output` folder that have not yet been created. Looking further down the `Makefile`, we see that these files have their own `make` rules. The rule to `make` the `report` itself, stipulates changing into the `code` directory[efn_note]This step is needed because R Markdown apparently likes to render documents in an R session with working directory set to the the directory where the `.Rmd` file lives.[/efn_note] and executing the `make_report.R` script, shown below. 
